...
# split in two lists
res_keys = [ c[0] for c in res_types ]
res_desc = [ c[1] for c in res_types ]

dia_full = [
    ('elgroup','Element Group','All','select',['All',]+self.DB.elems.keys()),
    ('resindex','Type of result',None,'select',res_desc),
...
# split in two lists
dia_keys = [ c[0] for c in dia_full ]
dia_items = [ c[1:] for c in dia_full ] 
dia_defaults = [ c[1] for c in dia_items ]
...
def open_results_dialog():
    global dialog
    for k,i in zip(dia_keys,dia_items):
        i[1] = data[k]
    actions = [('Close',close),('Reset',reset),('Show',show),             ]
    dialog = InputDialog(dia_items,caption='Results Dialog',actions=actions,default='Show')
    dialog.show()
