...
def firstWord(s):
    """Return the first word of a string, or the whole string.
...
class FlatDB(list):
    ...
    def __setslice__(self, i, j, reclist):
        """Sets slice of items [i,j] to the given list of records."""
        copy_list = reclist[:]
        for item in reclist:
            if not self.checkRecord(item):
    ...
    def insert(self, i, record):
        """Insert a record at position i in the database."""
        if self.checkRecord(record):
            list.insert(self, i, record)
    ...
    def match(self,key,value):
        ...
        This returns a list of record numbers.
        """
        return [ i for i in range(len(self)) if self[i].has_key(key) and
                 self[i][key] == value ]  
...
if __name__ == '__main__':
    ...
    print db[0]
    db[1] = { 'aa':'dd'}
    db.__setslice__(0,1,[{ 'aa':32, 'bb':31 }, {'aa':'no'}, {'dd':''} ])
    print db
    db.append({ 'ee':'ee'})
    db.insert(2,{'a':'b'})
    db[:0] = [ db.newRecord() for i in range(5) ]
    print db
    print len(db)
    
    mat = FlatDB(beginrec='material',endrec='endmaterial')
    ...
    mat = FlatDB(beginrec='material',endrec='')
    mat.readFile('materials.sng')
    print mat

    mat = FlatDB(beginrec='',endrec='')
