...
\htmladdnormallinkfoot{\pyformex website}{\websiteURL}. As of the writing of this manual, the latest release is \htmladdnormallinkfoot{\pyf \release}{\latestpyf}. 
...
Alternatively you can download the tarball releases from our \htmladdnormallinkfoot{local FTP server}{ftp://bumps.ugent.be/pub/pyformex\xspace}. The server may be slower, but occasionally you can find there an interim release or release candidate not (yet) available on the official server.
...
\Code{/usr/local/share/doc/pyformex-VERSION} is a link to the \Code{doc} subdirectory of pyformex, and \Code{/usr/local/bin/pyformex} is a link to the main executable \Code{pyformex.py}.
