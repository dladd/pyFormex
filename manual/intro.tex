...
\section{Running \pyformex}
\label{sec:running}
To start \pyf, enter the command \Code{pyformex}. This will start the \pyf Graphical User Interface (GUI), form where you can launch examples or load, edit and run your own \pyf scripts.
...
If you start \pyf with the \Code{--nogui} option, no Graphical User Interface is created. This is extremely useful to run automated scripts in batch mode. When run with the \Code{--nogui} option, \pyf will interprete all arguments remaining after interpreting the options, as filenames of scripts to be run (and possibly arguments to be interpreted by these scripts).
Thus, if you want to run a \pyf script \Code{myscript.py} in batch mode, just give the command \Code{pyformex --nogui myscript.py}.
